...
class ReportSemanticMetric(BaseMetric):
    ...
    def measure(self, test_case: LLMTestCase):
        ...
        llm = get_llm_model({"provider": "ollama", "model": "gemma4:31b"})
...
def run_case_with_tracing(case: dict) -> LLMTestCase:
    ...
    return LLMTestCase(
        ...
        expected_output=case.get("gold_sql", ""),
        metadata={
            "agent_sql": agent_sql,
...
def test_agent_case(case):
    ...
    assert_test(
        test_case=tc,
        metrics=[SqlCorrectnessMetric(), HallucinationMetric(), ToolRoutingMetric(), ReportSemanticMetric()],
        run_async=False,
    )
...
def main():
    ...
    for i, case in enumerate(CASES):
        ...
        sql_ok = hal_ok = True
        try:
            assert_test(test_case=tc, metrics=[sql_metric, hal_metric, ReportSemanticMetric()], run_async=False)
