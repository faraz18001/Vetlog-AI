...
@router.get("/user/")
def list_user_reports(user_id: int = Query(..., description="User ID to fetch reports for"),
                      db: Session = Depends(get_session)):
    """
    Return all reports and tables generated by a specific user,
    ordered by most recent first, deduplicated by filename.
    """
    logs = (
        db.query(ConversationLog)
        .filter(ConversationLog.user_id == user_id)
        .filter(
            (ConversationLog.report_path.isnot(None))
            | (ConversationLog.table_path.isnot(None))
        )
        .order_by(desc(ConversationLog.created_at))
        .all()
    )

    # Deduplicate by filename, keeping the most recent entry
    seen = set()
    reports = []
    for log in logs:
        for path_attr in ("report_path", "table_path"):
            path = getattr(log, path_attr)
            ...
            reports.append({
                "filename": filename,
                "type": "table" if path_attr == "table_path" else "report",
